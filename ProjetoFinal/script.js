...
var cardList = tasks ? JSON.parse(tasks) : []
criarCard()
// localStorage.clear()
var cont = 0
...
function criarTarefa() {
    ...
    localStorage.setItem('tasks',JSON.stringify(cardList))//transformei o JSON de cardlist em string para passar pro localstorage
    criarCard()
    fecharModal()
}
...
function criarCard() {
    ...
    cardList.forEach(tarefa => {
        ...
        const coluna = document.querySelector(`[data-column="${tarefa.status}"] .corpo .listaCard`)
        const card = `
        <div 
            id="${tarefa.id}"
            class="card" 
